...
var bfBtnFlag = false; // We used this flag to keep track of Back/Fwd Btn
...
function openCacheEntry(url, mode) {
  var session = createCacheSession(clientID, nsICache.STORE_ON_DISK, true);
    var entry = session.openCacheEntry(url, mode, false);
  return entry;
}
...
tabs.on('ready', function(evtTab) {
    if (evtTab.url != "about:blank") {  
     ...
     //   console.log("index="+evtTab.index);             
        ...
        // //@ X-Content-Security-Policy Header Retrieval from Cache 
        if (bfBtnFlag) {
          // try {
          //     bfBtnFlag = false;
          //   var inputEntry = openCacheEntry(hostName, nsICache.ACCESS_READ);
          //   var metadata = inputEntry.getMetaDataElement("X-Content-Security-Policy");
          //   inputEntry.close();
          //   console.log("\n\n Cache [X-Content-Security-Policy]="+metadata);
          // } catch (e) {
          //     console.log("\n Cannot get X-Content-Security-Policy header from the Cache");
          //   }
        }
        // //# X-Content-Security-Policy Header Retrieval from Cache
