...
var userCSPCacheListener = {
    ...
    onCacheEntryAvailable: function(descriptor, accessGranted, status) {
        ...
        if ((accessGranted == Ci.nsICache.ACCESS_READ) && (descriptor) && (status == Cr.NS_OK)) {
            ...
            console.log("Cache entry descriptor is not available.");
        }
  ...
  // onCacheEntryAvailable: function(descriptor, accessGranted, status) {
  //     console.log("\n\n I am inside onCacheEntryAvailable. entry = "+entry);
  //     console.log("\n Access = " + access);
  //     console.log("\n Status = " + status);
  ...
  //         var metadata = entry.getMetaDataElement("X-Content-Security-Policy");
  //         entry.close();
  //         console.log("\n\n Cache [X-Content-Security-Policy]="+metadata);
  //         //return entry;
  //     }
  // }
...
function openCacheEntry(url, mode) {
  var session = createCacheSession(clientID, nsICache.STORE_ANYWHERE, true);
  console.log("\n\n Double check URL = "+ url);
  session.asyncOpenCacheEntry(url, mode, userCSPCacheListener);
 // return entry;
}

// Hack to detect page loads from cache
...
const userCSP_progressListener = {
  onLocationChange: function (aWebProgress, aRequest, aLocationURI, aFlags) {
   ...
   // //read HTTP header (Content-Security-Policy) from cache
      // //@ Content-Security-Policy Header Retrieval from Cache         
       try {   
           var cacheKey = browser.contentWindow.location.href.replace(/#.*$/, "");
            openCacheEntry(cacheKey, Ci.nsICache.ACCESS_READ);          
          } catch (e) {
      ...
      userCSP_getRulesForOpenTabs(browser.contentWindow.location.href);
    ...
    if (found) {
        ...
        if (backbtn) {
           // console.log ("\n Back button found ="+backbtn);
            backbtn.addEventListener("click", backOrForwardBtnClicked, false);
        }
        var forwardbtn = active[winI].document.getElementById("forward-button");
        if (forwardbtn) {
            forwardbtn.addEventListener("click", backOrForwardBtnClicked, false);
        }
...
function userCSP_getRulesForOpenTabs(hostName) {
    ...
    for (var i=0; i<tabs.length; i++) {
        ...
        if (!searchStringInArray(tempStr)) { 
            ...
            console.log("Infered rules for "+tempStr+" ="+Result);
        } // end of IF loop
    } // end of FOR loop
