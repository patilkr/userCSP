...
function getDomainRulesFromDB(dList) {
    ...
    query.executeAsync({
            handleResult:function(resultSet){
                ...
                // console.log("\n Select query sqrObject.rows="+sqrObject.rows);
...
function insertValInDB(domainName, directiveRules) {
    ...
    let stmt = connection.createStatement("INSERT INTO  userCSPTable(value) VALUES(:value);");
    let params = stmt.newBindingParamsArray();
    for (let i = 0; i < 16; i++) {
        if (i == 0) {
            let bp = params.newBindingParams();
            bp.bindByName("value", domainName);
            params.addParams(bp);
        } else {
            let bp = params.newBindingParams();
            bp.bindByName("value", directiveRules[i-1]);
            params.addParams(bp);
        }
    }
    stmt.bindParameters(params);
    connection.executeSimpleSQL(stmt);
...
function storeDBintoMyData(domainName, directiveRules) {
    ...
    var statement = connection.createStatement("SELECT * FROM table_name WHERE id = :row_id");
    statement.params.row_id = domainName;
...
query.executeAsync({
        handleResult:function(resultSet){
            ...
            for(var i=0;i<sqrObject.rows;i++){
                ...
                myDatabase[sqrObject.data[i][0]][2] = sqrObject.data[i][13];
                myDatabase[sqrObject.data[i][0]][3] = sqrObject.data[i][14];
                myDatabase[sqrObject.data[i][0]][4] = sqrObject.data[i][15];
