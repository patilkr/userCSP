...
userCSP.port.on("getCSPforAll", function (text) {
    ...
    timers.setTimeout(function(){dump("\n I have got record for : "+domainNameRules.length+" Domain in main add-on \n"); if (domainNameRules.length != 0) { console.log("I need to send data to the add-on UI for : "+domainNameRules.length+ " Domain names"); userCSP.port.emit("showCSPRules",domainNameRules,websiteCSPRules,websiteCSPFull,myDatabase);} }, 1000);
