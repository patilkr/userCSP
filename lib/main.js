...
userCSP.port.on("storeUserCSPUIState", function (selectedDomain, userCSPUIState, userCSPAll, userCSPArray) {    
    var result = userCSPAll[selectedDomain];
    
    ss.storage.userCSPState[selectedDomain] = userCSPUIState[selectedDomain];
    if (!ss.storage.userCSPPolicies[selectedDomain])
        ss.storage.userCSPPolicies[selectedDomain] = result;
...
console.log("\n\n userCSPPolicies[http://www.attacker.com]=" + ss.storage.userCSPPolicies[activeWindow]);

    // send record to add-on UI
    timers.setTimeout(function (){       
      userCSP.port.emit("showCSPRules",  activeWindow, websiteCSPFull, websiteCSPRules,  ss.storage.userCSPPolicies, ss.storage.userCSPPoliciesArray, ss.storage.inferredPolicies, ss.storage.inferredPoliciesArray);
    }, 1000);	  
