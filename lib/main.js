...
// * Tab testing
// */
// let xpcomUtil = Cu.import("resource://gre/modules/XPCOMUtils.jsm");
...
//         var channel = ios.newChannel("test.html", null, null);
...
//     classID: components.ID("{8c559f03-afe0-4aac-91ff-6c215895d467}"),
//     contractID: "@mozilla.org/network/protocol/about;1?what=mystuff",
//     QueryInterface: XPCOMUtils.generateQI([Ci.nsIAboutModule])
// }

// function NSGetModule(aCompMgr, aFileSpec) {
//   return xpcomUtil.generateModule([MyAboutHandler]);
// }

// XPCOMUtils.generateNSGetFactory([MyAboutHandler]);

// // Tab Testing code ends here
...
// var gctest = "Hello world!!!";
// function test(context, hostName) {
//     return gctest;   
// }
...
userCSP.port.on("getCSPforAll", function (text) {
    ...
    timers.setTimeout(function() {
        ...
        if (domainNameRules.length != 0) { 
            console.log("I need to send data to the add-on UI for : " + domainNameRules.length + " Domain names"); 
            userCSP.port.emit("showCSPRules", domainNameRules, websiteCSPRules, websiteCSPFull, myDatabase, ss.storage.inferredPolicies);
        } 
    }, 1000);
...
function userCSP_getRulesForOpenTabs(hostName) {
    ...
    // send record to add-on UI
	  timers.setTimeout(function (){
        dump("\n I have got record for : "+domainNameRules.length+" Domain in main add-on"); 
        if (domainNameRules.length != 0) { 
            console.log("\n I need to send data to the add-on UI for : "+domainNameRules.length+ " Domain names"); 
            userCSP.port.emit("showCSPRules", domainNameRules, websiteCSPRules, websiteCSPFull, myDatabase, ss.storage.inferredPolicies);
        }else{
            console.log("\n I need to send data to the add-on UI. showCSPRules : "); 
            userCSP.port.emit("showCSPRules", domainNameRules, websiteCSPRules, websiteCSPFull, myDatabase, ss.storage.inferredPolicies); 
        } 
    }, 1000);
        ...
        handleCompletion:function(reason){
                ...
                for(var i = 0; i < sqrObject.rows; i++){
                    for(var j = 1; j <= 11; j++){
                        ...
                        if (sqrObject.data[i][j] == "" || sqrObject.data[i][j] == " "|| sqrObject.data[i][j] == null) continue;
                        
                        Result = userCSPFilterMod.addCSPDirectiveNames(Result, j, sqrObject.data[i][j]);
                   ...
                   // myDatabase[domainName][4] = sqrObject.data[i][15]; // eval
                    console.log ("\n store: myDatabase[domainName][0] = " + myDatabase[domainName][0]);
                    console.log ("\n store: myDatabase[domainName][3] = " + myDatabase[domainName][3]);
        ...
        handleResult:function(resultSet){
            ...
            for(var i = 0; i < sqrObject.rows; i++) {
                for(var j = 1; j <= 11; j++) {
                    ...
                    dbResult = userCSPFilterMod.addCSPDirectiveNames(dbResult, j, sqrObject.data[i][j]);
                } // end of For J loop
     
               // console.log("###Debug: Result = " + dbResult);

                //Record data of all domains
                myDatabase[sqrObject.data[i][0]] = new Array(5);
                myDatabase[sqrObject.data[i][0]][0] = dbResult; 
              ...
              //  myDatabase[sqrObject.data[i][0]][4] = sqrObject.data[i][15]; // inline-eval
               // console.log ("\n myDatabase[sqrObject.data][0] = "+myDatabase[sqrObject.data[i][0]][0]);
               // console.log ("\n myDatabase[sqrObject.data][3] = "+myDatabase[sqrObject.data[i][0]][3]);
               
                dbResult = ""; // reset Result for next row of data
            } // end of FOR I loop
        ...
        if (typeof(myDatabase[responseName]) === 'undefined') {
            if (typeof(myDatabase["all"]) === 'undefined') {
                if (typeof(myDatabase[hostName]) === 'undefined') {
                    console.log ("\n Everything is UNDEFINED so didn't set HTTP header");
                    return;
        ...
        if ((myDatabase[responseName]) == null) {
            if ((myDatabase["all"]) == null) {
                if ((myDatabase[hostName]) == null) {
                    console.log ("\n Everything is NULL so didn't set HTTP header");
                    return;
                }
            }
        }

       // console.log (" \n myDatabase[0]="+myDatabase[responseName][0]);
       // console.log (" \n myDatabase[3]="+myDatabase[responseName][3]);
        ...
        try {       
            if (typeof(myDatabase[responseName]) !== 'undefined') {
                ...
                console.log ("\n Some condition is missing. Logical error!!");
            }
...
userCSP.port.on("inferCSPArray", function (webDomain) {
    userCSP.port.emit("setInferAsUserCSP",  webDomain, ss.storage.inferredPoliciesArray);
});

// send Infer CSP Policy Array to addon
userCSP.port.on("inferCSPArrayFormat", function (webDomain) {
    userCSP.port.emit("setInferCSPArray",  webDomain, ss.storage.inferredPoliciesArray);
});
