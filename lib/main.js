...
gcl.gcli.addCommand({
    ...
    exec: function(args, context) {
        ...
        return "Content-Security-Policy header set to "+ ss.storage.inferredPolicies[hostName];
        //return test();
    }
});
...
userCSP.port.on("getCSPforAll", function (text) {
    ...
    timers.setTimeout(function(){dump("\n I have got record for : "+domainNameRules.length+" Domain in main add-on \n"); if (domainNameRules.length != 0) { console.log("I need to send data to the add-on UI for : "+domainNameRules.length+ " Domain names"); userCSP.port.emit("showCSPRules",domainNameRules,websiteCSPRules,websiteCSPFull,myDatabase, ss.storage.inferredPolicies);} }, 1000);
...
function userCSP_getRulesForOpenTabs(hostName) {
    ...
    // send record to add-on UI
	  timers.setTimeout(function(){dump("\n I have got record for : "+domainNameRules.length+" Domain in main add-on"); if (domainNameRules.length != 0) { console.log("\n I need to send data to the add-on UI for : "+domainNameRules.length+ " Domain names"); userCSP.port.emit("showCSPRules",domainNameRules,websiteCSPRules,websiteCSPFull,myDatabase,ss.storage.inferredPolicies);}else{console.log("\n I need to send data to the add-on UI. showCSPRules : "); userCSP.port.emit("showCSPRules",domainNameRules,websiteCSPRules,websiteCSPFull,myDatabase, ss.storage.inferredPolicies); } }, 1000);
...
// } // end of userCSPNullFilter function
